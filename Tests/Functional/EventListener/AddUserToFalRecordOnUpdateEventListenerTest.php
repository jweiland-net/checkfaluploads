...
use JWeiland\Checkfaluploads\EventListener\AddUserToFalRecordOnUpdateEventListener;
 ...
 * Test case.
 */
class AddUserToFalRecordOnUpdateEventListenerTest extends FunctionalTestCase
{
    ...
    public function invokeStoresBackendUserOnFileWhenBackendStorageIsEnabled(): void
    {
        ...
        $subject = new AddUserToFalRecordOnUpdateEventListener(
            new Context(),
            new ExtConf(storeBackendUploaderUserId: true),
        );

        $subject->__invoke($this->createEvent());
    ...
    private function createEvent(): AfterFileUpdatedInIndexEvent
    {
        $file = new File(
            [
        ...
        return new AfterFileUpdatedInIndexEvent($file, ['uid' => 1], []);
    }
