 ...
 * Test case.
 */
class ElementBrowserUploadRightsCheckMiddlewareTest extends FunctionalTestCase
{
    ...
    #[Test]
    public function processSkipsLoadingJavaScriptModuleWhenElementBrowserUploadRightsCheckIsDisabled(): void
    {
        $pageRenderer = $this->createMock(PageRenderer::class);
        $pageRenderer->expects($this->never())->method('loadJavaScriptModule');

        $subject = new ElementBrowserUploadRightsCheckMiddleware(
            $pageRenderer,
            new ExtConf(checkElementBrowserUploadRights: false),
        );

        $subject->process($this->createRequestForElementBrowserRoute(), $this->createRequestHandler());
    }

    #[Test]
    public function processLoadsJavaScriptModuleForElementBrowserRouteWhenEnabled(): void
    {
        $pageRenderer = $this->createMock(PageRenderer::class);
        $pageRenderer
            ->expects($this->once())
            ->method('loadJavaScriptModule')
            ->with('@jweiland/checkfaluploads/element-browser-upload-rights-check.js');
    ...
    private function createRequestForElementBrowserRoute(): ServerRequestInterface
    {
