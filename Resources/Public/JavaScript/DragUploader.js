 ...
 ***************************************************************/
/**
 * JavaScript RequireJS module called "TYPO3/CMS/Checkfaluploads/DragUploader"
 *
 */
define('TYPO3/CMS/Checkfaluploads/DragUploader', ['jquery'], function($) {
	...
	var DragUploaderPlugin = function(element) {
		...
		me.$checkboxForRights = $('<input type="checkbox" id="checkFalUploads" value="1" />');
		me.$checkForFalUploads = $('<div />').append(me.$checkboxForRights).append('<label for="checkFalUploads">' + TYPO3.l10n.localize('dragUploader.iHaveTheRights') + '</label>').hide();
		me.$dropzone.before(me.$checkForFalUploads);
		...
		me.showDropzone = function() {
			me.$dropzone.show();
			me.$checkForFalUploads.show();
		};

		me.hideDropzone = function(event) {
			me.$checkForFalUploads.hide();
			me.$checkForFalUploads.hide();
		...
		if (me.browserCapabilities.DnD) {
			...
			me.$dropzone.prepend(
				'<h4>'+TYPO3.l10n.localize('file_upload.dropzonehint.title')+'</h4>' +
				'<p>'+TYPO3.l10n.localize('file_upload.dropzonehint.message')+'</p>'
			).click(function() {
				me.$fileInput.click()
			});
	...
	var FileQueueItem = function(dragUploader, file, override) {
		...
		if (me.file.size > me.dragUploader.maxFileSize) {
			...
			if(dragUploader.$checkboxForRights.is(':checked')) {
				formData.append('userHasRights', '1');
			} else {
				formData.append('userHasRights', '0');
			}
	...
	var initialize = function() {
		...
		if (undefined !== TYPO3.settings && undefined !== TYPO3.settings.RequireJS && undefined !== TYPO3.settings.RequireJS.PostInitializationModules && undefined !== TYPO3.settings.RequireJS.PostInitializationModules['TYPO3/CMS/Checkfaluploads/DragUploader']) {
			$.each(TYPO3.settings.RequireJS.PostInitializationModules['TYPO3/CMS/Checkfaluploads/DragUploader'], function(pos, moduleName) {
				require([moduleName]);
			});
		}
